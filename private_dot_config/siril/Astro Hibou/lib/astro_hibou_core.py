...
# Absolute path to the SyQon logging wrapper, which lives beside this module in
# lib/ so it stays out of Siril's Scripts menu (the menu lists only top-level
# files of a script_path dir).
#
# It MUST be passed to `pyscript` as an absolute path, and it must be QUOTED.
# Two independent traps, both verified against the 1.5 source on 2026-07-19:
#
#   1. `process_pyscript` (src/core/command.c:15046) uses an argument verbatim
#      if it stats as a file; otherwise it probes each script_path entry with
#      find_file_in_directory() (src/core/utils.c:1650) — a single
#      g_build_filename(path, basename) + stat, with NO recursion. Only the
#      community repo gets find_file_recursively(). So a bare
#      "syqon_logged.py" was resolvable only while this file sat at the top
#      level; from lib/ it would not be found at all.
#   2. The script directory is "Astro Hibou", which contains a space, and
#      parse_line (src/core/command_line_processor.c:97) splits on blanks
#      unless the token is wrapped in " or '. Unquoted, Siril would receive
#      ".../Astro" and fail. This is the same trap that silently discarded the
#      Ha term from every pm expression until 2026-07-09 — see pixel_math().
SYQON_WRAPPER = Path(__file__).resolve().parent / "syqon_logged.py"
...
class Pipeline:
    ...
    def _run_syqon(
        ...
        self.siril.cmd("pyscript", f'"{SYQON_WRAPPER}"', tool, *args)
