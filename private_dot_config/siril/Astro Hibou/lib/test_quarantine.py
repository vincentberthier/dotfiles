...
print("\n=== meridian flip (must be kept) ===")
# A cross-night meridian flip (pier West vs East) rotates the field ~180 deg.
# The IMX533 is square, so a ~180 deg rotation about the field centre still
# overlaps 93-100 % and homography registration aligns it fine — it must NOT be
# disposed. This is the NGC 7023 2026-07-13/14 case (1 flipped Blue sub pooled
# with 2 from the other night) that the old rotation>1deg threshold wrongly
# discarded; the disposed sub measured 170.76 deg / ~93 % overlap on sky.
out,g = run("3 frames, 1 flipped ~171deg", [rot(170.764),I,I]); assert not out and not g, "meridian flip must be kept"
out,g = run("4 frames, 1 flipped 180 exact", [rot(180),I,I,I]);  assert not out and not g
out,g = run("5 frames, 2 flipped (both nights)", [rot(179.5),rot(180.5),I,I,I]); assert not out and not g
# ...but a genuine ~135 deg misframe (only 83 % overlap) is still caught.
out,g = run("4 frames, 1 at 135deg (83% overlap)", [rot(135),I,I,I]); assert set(out)=={1} and not g
